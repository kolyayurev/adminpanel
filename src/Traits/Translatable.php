...
trait Translatable
{
    ...
    {
        return $this->hasMany(AdminPanel::model('Translation'), 'foreign_key', $this->getKeyName())
            ->where('table_name', $this->getTable())
            ->whereIn('locale', config('adminpanel.multilingual.locales', []));
    }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('adminpanel.multilingual.default'), false];
        }
        ...
        $default = config('adminpanel.multilingual.default');
    ...
    public function isTranslatableAttribute($name): bool
    {
        return in_array($name, $this->getTranslatableAttributes());
    }
...
//                throw new Exception('Invalid Translatable field '.$field);
            ...
            $request->merge([$field => $trans[config('adminpanel.multilingual.default', 'ru')]]);
    ...
    public function prepareTranslationsFromArray($field, &$requestData)
    {
        ...
        // Set the default local value
        $requestData['display_name'] = $trans[config('adminpanel.multilingual.default', 'ru')];
