...
AdminPanel supports multiple languages for your models. To get started, you need to configure some things first.

## Setup

First you need to define some `locales` in your `config/adminpanel.php` file and `enable` multilanguage:
...
Now you will see a language-selection in your content page.
...
```php
// with string
if (is_translatable(Post::class)) {
    // it's translatable
}

// with object of Model or Collection
if (is_translatable($post)) {
    // it's translatable
}
```
