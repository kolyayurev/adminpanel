...
Только для просмотра: значение колонки, скастованной в `array` (json/jsonb), сериализуется
для отображения (`JSON_PRETTY_PRINT`) — в списке обрезается по 50 символов, на странице
записи выводится целиком в `<pre>`. По умолчанию скрыто в списке и формах создания/
редактирования (`hiddenOn(['list', 'create', 'edit'])`) — редактирования "в лоб" текстом
поле не даёт:

```php
Json::make('meta')->label('Метаданные');
```
...
`Select` для колонки, скастованной в `BackedEnum` (`casts(): array { return ['status' =>
Status::class]; }`). Разворачивает enum-объект в скаляр при чтении и умеет собрать опции
из самого класса enum — словарь руками собирать не нужно:

```php
EnumSelect::make('status')->enum(Status::class);
```

Подпись варианта — из метода `label()` на enum, если он есть, иначе — из имени кейса
(`$case->name`). При сохранении конвертация не нужна: Eloquent-каст сам принимает скаляр
и превращает его обратно в enum.
